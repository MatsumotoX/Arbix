...
<script>
    ...
    export default {
        ...
        <div class="form-row">
            <div class="form-group col-md-12">
                <eec-inputbox :value.sync="data.quickReplyButton" placeholder="Quick Reply Button" allowfilter="true" filtertype="contains"
                              @input="buttonSelected($event)" type="multiselect" :option="gridquickreply.column[2].option" mode="CheckBox"
                              ></eec-inputbox>
            </div>
            <div class="form-group col-md-12" v-show="false">
               <div class="e-float-input e-control-wrapper">
                    <input id="quickReplyButton" name="quickReplyButton" v-model='data.quickReplyButton' type="text">
                    <span class="e-float-line"></span>
                    <label class="e-float-text e-label-top" for="content_id">content_id</label>
                                ...
                                created() {
                                    this.gridquickreply = quickReply;
                                    axios['get']('./getData')
                                        .then(response => {
                                            this.gridquickreply = response.data.quickReply;
                                        })
                                        .catch(error => {
                                            console.log(error);
                                        });
                                ...
                                methods: {
                                    buttonSelected: function (args) {
                                        this.data.quickReplyButton = args;
                                    },
                                }
                            })
                        }
        ...
        methods: {
            ...
            updateData: function (args) {
                ...
                switch (args.requestType) {
                    case  'save':
                        // console.log(args);
                        args.data.quickReplyButton = JSON.stringify(args.data.quickReplyButton.split(','));
