...
<script>
    ...
    export default {
        ...
        <div class="form-row">
            <div class="form-group col-md-12">
                <eec-inputbox :value.sync="data.bound_id" placeholder="Bound" allowfilter="true" filtertype="contains"
                              @input="boundSelected($event)" type="select" :option="gridareas.column[2].option"></eec-inputbox>
            </div>
        </div>
        <div class="form-row">
            <div class="form-group col-md-12">
                <eec-inputbox :value.sync="data.action_id" placeholder="Action" allowfilter="true" filtertype="contains"
                              @input="actionSelected($event)" type="select" :option="gridareas.column[3].option"></eec-inputbox>
            </div>
            <div class="form-group col-md-12" v-show="false">
               <div class="e-float-input e-control-wrapper">
                    <input id="bound_id" name="bound_id" v-model='data.bound_id' type="text">
                    <span class="e-float-line"></span>
                    <label class="e-float-text e-label-top" for="bound_id">bound_id</label>
                </div>
            </div>
            <div class="form-group col-md-12" v-show="false">
               <div class="e-float-input e-control-wrapper">
                    <input id="action_id" name="action_id" v-model='data.action_id' type="text">
                    <span class="e-float-line"></span>
                    <label class="e-float-text e-label-top" for="action_id">action_id</label>
                </div>
            </div>
        </div>
        </div>`,
                                ...
                                created() {
                                    ...
                                    axios['get']('./getData')
                                        .then(response => {
                                            this.gridareas = response.data.areas;
                                        })
                                        .catch(error => {
                                            console.log(error);
                                        });
                                ...
                                methods: {
                                    boundSelected: function (args) {
                                        this.data.bound_id = args;
                                    },
                                    actionSelected: function (args) {
                                        this.data.action_id = args;
                                    }
                                }
                            })
                        }
        ...
        methods: {
            ...
            updateData: function (args) {
                ...
                switch (args.requestType) {
                    case  'save':
                        args.data['bound_id'] = args.data['bound']['id'];
                        args.data['action_id'] = args.data['action']['id'];
                        delete args.data['null'];
                        delete args.data['bound'];
